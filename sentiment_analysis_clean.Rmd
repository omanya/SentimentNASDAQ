...
ean<-transform(ean,surprise=NA)
ind<-!is.na(ean$eps_est)&!is.na(ean$eps)&ean$eps_est!=0&ean$eps!=0
ean$surprise[ind]<-((ean$eps-ean$eps_est)/ean$eps)[ind]
ean$surprise[is.na(ean$surprise)]<-0
#correct for outliers
ean$suprise[ean$surprise>quantile(ean$surprise,0.99,na.rm=T)]<-quantile(ean$surprise,0.99,na.rm=T)
ean$suprise[ean$surprise<quantile(ean$surprise,0.01,na.rm=T)]<-quantile(ean$surprise,0.01,na.rm=T)
ean$surprise<-scale01(ean$surprise)
data<-merge(data,ean[,c("symbol", "date","ean","surprise","eps") ], by.x=c("Date","Ticker"),by.y=c("date","symbol"),all.x=T)
...
#data_agg<-cbind(aggregate(ean~Sector,data[ind_use,],sum),aggregate(post_ean~Sector,data[ind_use,],sum)[,2])
...
```{r,echo=FALSE}
#I(turnover*news) effect of different opinions 

ind_use<-which(!is.na(data$btm1))
formul<-as.formula("r_.ab.~ pre_ean+ean+post_ean+news+news_sector+news_day+log(Size)+ btm1 + I(turnover1*days_ean)+ I((btm1<1)*post_ean) +I(surprise*100) + eps+ LM.TONE + HIV4.TONE +Sector+Ticker")
...
make_knitr_table_all(mod0, which_tau=which_tau,taus=taus_mod, which_ones = 1:15,
                     ...
                     row.names=c("Constant","pre_ean_{i,t}","ean_{i,t}","post_ean_{i,t}",
                                 ...
                                 "(btm_{i,t-1}< 1)*post_ean_{i,t}","supr_{i,t}", "eps_{i,t}",
...
plot_eefects(mod0,which_ones=wo,which_tau=which(taus_mod%in%seq(0.05,0.95,0.05)),lbls=lbls[c(1:2, 4:5)],sc=c(2,3,1,1),cex.axis=0.7)
...
plot_eefects(mod0,which_ones=wo,which_tau=which(taus_mod%in%seq(0.05,0.95,0.05)),lbls=lbls[1:length(wo)],
             sc=c(0.5,1,1,4),cex.axis=0.7)

```

<!--The effects of $btm_{i,t-1}$ and turnover around earnings announcements appear to be negative for low expectiles and become increasingly positive for higher $\tau$ indicating their particular importance in extreme states of the market. $\log(size)$ exhibits increasingly negative influence for higher return expectiles.

According to the results, the returns of firms with small btm ratio smaller than one and low return states undergo a negative correction in the post earnings day.-->
...
al<-0.95
wo<-c("I(surprise * 100)","LM.TONE","HIV4.TONE")#, "eps")#,"ML.TONE")

plot_eefects(mod0,which_ones=wo,which_tau=which(taus_mod%in%seq(0.05,0.95,0.05)),lbls=lbls[1:length(wo)],
             special=cbind(c(-0.02,-0.002,-0.004,-0.002),c(0.04,0.004,0.008,0.004)),sc=c(1,1,1,1),cex.axis=0.7)
...
We take $\tau=0.1,0.25$ for our analysis which are also used in @eberl2022sk and consider the straight forward adjustment for our conditional expectile regression setup of the model in \@ref(eq:mod0). In our case, the estimated conditional interexpectile range becomes for $\tau=0.1$ (for $\tau=0.25$ analogous):
...
where $x_{i,t}$ is a vector of the time-varying firm characteristics from \@ref(eq:mod0) and $z_i$ is a vector of firm characteristics which do not change over time. Similar, estimated conditional expectile-based skewness measure $\hat s_{\tau,i,t}$ is for $\tau=0.1$ (for $\tau=0.25$ analogous):
...
For each firm and each period, we compute $\hat\Delta_{0.1,i,t}$ . Then, we average the resulting values considering time indices $t$, at which different events occur, separately. The resulting means are denoted as $\overline\Delta_{0.1}^{no~event}$ and $\overline\Delta_{0.1}^{event}$. The associated relative changes in the interexpectile ranges for particular events result from normalizing to the case of no events, $\overline\Delta^{event}_{0.1}/\overline\Delta^{no~event}_{0.1}$ and are provided in Table \@ref(tab:tabdelt1). 
...
<!--
Assuming the independence of returns on the distinct event days\footnote{Because the times between the events normally encount many days, we assume that the corresponding returns are nearly independend.} we can construct asymptotic confidence intervals for the skewness measures based on the result of Theorem 4 in @eberl2022sk:
\[\hat s_{\tau} - \frac{\hat\sigma_{\tau}}{\sqrt{n}}z_{1-p/2}\leq s_{\tau}\leq \hat s_{\tau} + \frac{\hat\sigma_{\tau}}{\sqrt{n}}z_{1-p/2}\]

where 
\[\begin{align}
\hat\sigma_{\tau}^2=&\frac4{(1-2\tau)^2}\Big[\frac{4\hat\eta(0.5,0.5)}{\hat\Delta_\tau^2} - \frac{4\{\hat A(\tau)\hat\eta(\tau,0.5) + \hat A(1-\tau)\hat\eta(0.5,1-\tau)\}}{\hat\Delta_\tau^3} 
\nonumber\\&+ \frac{\hat A(\tau)^2\hat\eta(\tau,\tau) 
+ \hat A(\tau)\hat A(1-\tau)\hat\eta(\tau,1-\tau) + \hat A(1-\tau)^2\hat\eta(1-\tau,1-\tau)}{\hat\Delta_\tau^4}\Big]
\end{align}\]
-->
...
# returns<-rnorm(100)
# tau=0.25
# etau<-expectreg::expectile(returns,tau)
# e1_tau<-expectreg::expectile(returns,1-tau)
# e0.5<-expectreg::expectile(returns,0.5)
#
# A<-function(tau,etau,e1_tau,e0.5,returns){
#       edf_etau<-mean(returns<=etau,na.rm=T)
#       out<-etau
#       out[!is.na(etau)]<-2*((tau<0.5)-1)*((e1_tau - e0.5)[!is.na(etau)])/(tau + edf_etau*(1-2*tau))
#     return(out)
# }
# #A(tau,etau,e1_tau,e0.5,returns)
# #A(0.25,data$etau,data$e1_tau,data$e0.5,data$r_.ab.)
# 
# eta<-function(etau1,etau2,tau1,tau2,returns){
#    out<-mean(
#     (tau1*mean((returns-etau1)[!is.na(etau1)&returns>=etau1])+(1-tau1)*mean((etau1-returns)[!is.na(etau1)&returns<etau1]))*
#     (tau2*mean((returns-etau2)[!is.na(etau1)&returns>=etau2])+(1-tau2)*mean((etau2-returns)[!is.na(etau1)&returns<etau2])),na.rm=T)
# return(out)
# }
# 
# #eta(data$etau,data$e1_tau,0.25,0.75,data$r_.ab.)
# 
# 
# sig2<-function(tau,data){
#     delta<-data$delta
#     etau=data$etau; e1_tau=data$e1_tau; e0.5=data$e0.5
#     returns=data$r_.ab.
#    Atau<-A(tau,etau,e1_tau,e0.5,returns)
#    A1_tau<-A(1-tau, e1_tau, etau,e0.5,returns)
#    term1<-4*eta(e0.5,e0.5,0.5,0.5,returns)/delta^2
#    term2<-4*(Atau*eta(etau,e0.5,tau,0.5,returns) + A1_tau*eta(e0.5,e1_tau,0.5,1-tau,returns))/delta^3
#    term3<-(Atau^2*eta(etau,etau,tau,tau,returns) + Atau*A1_tau*eta(etau,e1_tau,tau,1-tau,returns) + A1_tau^2*eta(e1_tau,e1_tau,1-tau,1-tau,returns))/delta^4
#    out<-4/((1-2*tau)^2)*(term1 - term2 + term3)
#    return(out)
# 
# }
# 
...
# #sig2(tau=0.25,data)
# 
# #data<-transform(data,sig2=sig2(0.25,data))
# 
# for(i in 1:nrow(df_agg)){
#   indi<-!is.na(data$type2)&data$type2==df_agg$event[i]
#   data$sig2[indi]<-sig2(taus_delta[1],data[indi,])
# 
# }
# 
# p<-0.1
# zp<-qnorm(1-p/2)
# df_agg<-cbind(aggregate(sk~type2,data,mean),sig2=aggregate(sig2~type2,data,mean)[,2],n=aggregate(sig2~type2,data,length)[,2])
# df_agg<-transform(df_agg,sk_low=sk-sqrt(sig2/n)*zp,sk_high=sk+sqrt(sig2/n)*zp)
# df_agg
```

<!--
\[\hat A(\tau) = (2\cdot\mathbf 1_{\tau<0.5} - 1)\frac{\hat e_{1-\tau} - \hat e_{0.5}}{\tau + \hat F(\hat e_\tau)(1-2\tau)}\]

with $\hat F$ the empirical cumulative distribution function, 

and

\[\hat\eta(\tau_1,\tau_2)=\frac 1n\sum_{j=1}^n \left\{\tau_1(r_j-\hat e_{\tau_1})\mathbf 1_{r_j\geq\hat e_{\tau_1}} - (1-\tau_1)(\hat e_{\tau_1}-r_j)\mathbf 1_{r_j<\hat e_{\tau_1}}\right\}\cdot \left\{\tau_2(r_j-\hat e_{\tau_2})\mathbf 1_{r_j\geq\hat e_{\tau_2}} - (1-\tau_2)(\hat e_{\tau_2}-r_j)\mathbf 1_{r_j<\hat e_{\tau_2}}\right\}\]

with $\{r_j\}_{j=1}^n$ the set of returns at particular event times.
-->
...
compute_sign<-function(df,levels=c(0.001,0.01,0.05),two_sided=T){
  dfm<-as.matrix(df[,-1])
  if(two_sided){
    levels=levels/2
  }
...
```{r, tabdelt1, echo=F}
caption<-"Means of boostrapped interexpectile ranges $\\overline\\Delta^{event}_{0.1}$ on event days and means of boostrapped expectile-based skewness $\\hat s_{0.1}$. The increases in the dispersion associated with $\\overline\\Delta^{event}_{0.1}$ compared to the case of no event ($\\overline\\Delta^{event}_{0.1}-\\overline\\Delta^{no~event}_{0.1}$) are given in brackets.  Symbols '***','**','*' indicate significance on 0.1\\%, 1\\%, 5\\% level respectively obtained based on bootstrap confidence intervalls."
...
rownames(tab)<-c("Constant","$pre\\_ean_{i,t}$","$ean_{i,t}$","$post\\_ean_{i,t}$",
                 ...
                 "$(btm_{i,t-1}\\leq 1)*post\\_ean_{i,t}$", "surp_{i,t}",
                                           "$eps_{i,t}$",  "$lm\\_tone_{i,t}$", "$hv\\_tone_{i,t}$")
...
```{r echo=FALSE}
#labs<-c("pre_ean","ean","post_ean","news","news_sector","news_day","Size", "btm1", "turnover1","surprise", "eps", "LM.TONE", "HIV4.TONE")
#cor(data[ind_use,labs])
```
...
<!--
## Test results for sentiment measures [-]

Following @LOUGHRAN2011 and @FRANKEL2022, equation (\ref{Eq:paneldataregression}) is used to examine if the sentiment measures explain the abnormal returns at the date of the news article publication. 
\[
\begin{aligned}
r_{i,t} = \alpha_0\ +\ \alpha_1sentiment_{i,t}\ 
& +\ \alpha_2\log{(size_{i,t})}\ +\ \alpha_3btm_{i,t}  \\
& +\  \alpha_4turn_{i,t}\ +\ \alpha_5prealpha_{i,t}\   \\ 
& +\ \alpha_6NASDAQ_{i,t}\ +\ \varepsilon _{i,t}, \\
\end{aligned}
\label{Eq:paneldataregression}
\]  
with $r_{i,t}$ the abnormal return at $t$ of stock $i$, $sentiment_{i,t}$ the corresponding sentiment measure at $t$ of stock $i$, $size_{i,t}$ the market value, $btm_{i,t}$ the book-to-market ratio, $turn_{i,t}$ the share turnover, $prealpha$
the Fama French 3 factor model intercept, $NASDAQ_{i,t}$ the NASDAQ dummy variable, and $\varepsilon _{i,t}$ the error term.

Table \ref{Tab:paneldm} shows the regression results with dictionary-based methods, and demonstrate that dictionary-based sentiment measures have significantly positive effects on the abnormal return. Table \ref{Tab:panelsml} displays the regression results with supervised machine learning methods, and illustrates that supervised machine learning sentiment measures also significantly influence the abnormal return with positive coefficients. Accordingly, we select three sentiment measures with the corresponding models achieving the highest adjusted $R^2$. Hence we choose the random forest sentiment with FinBERT, and dictionary-based sentiment measures with the LM and HV dictionary respectively. 

\begin{table}[!htbp] \centering
\caption{Regression results: dictionary-based methods }
\label{Tab:paneldm}
\small
\begin{tabular}{@{\extracolsep{5pt}}lcc}
\\[-1.8ex]\hline
\hline \\[-1.8ex]
& \multicolumn{2}{c}{\textit{Dependent variable: $r_{i,t}$}} \
\cr \cline{2-3}
\\[-1.8ex] & (1) & (2) \\
\hline \\[-1.8ex]
  HV sentiment & 0.001$^{***}$ & \\
  ...
  log(size) & 8.6e-06$^{}$ & 4.7e-05$^{}$ \\
  & (-8.1e-05 , 9.8e-05) & (-4.1e-05 , 0.000) \\
  btm & 2.5e-05$^{}$ & 2.7e-05$^{}$ \\
  & (-4.1e-05 , 9.1e-05) & (-4.0e.05 , 9.3e-05) \\
  turn & -4.7e-05$^{}$ & -3.1e-05$^{}$ \\
  & (-0.000 , 1.8e-05) & (-9.6e-05 , 3.4-05) \\
  prealpha & 0.118$^{***}$ & 0.102$^{**}$ \\
  & (0.034 , 0.202) & (0.018 , 0.186) \\
 NASDAQ & 3.0e-04$^{***}$ & 3.0e-04$^{***}$ \\
  & (0.000 , 0.001) & (0.000 , 0.001) \\
\hline \\[-1.8ex]
 Observations & 111,300 & 111,300 \\
 $R^2$ & 0.037\% & 0.073\% \\
 Adjusted $R^2$ & 0.031\% & 0.067\% \\
 F Statistic & 6.785$^{***}$ (df = 6.0; 111293.0) & 13.500$^{***}$ (df = 6.0; 111293.0) \\
\hline
\hline \\[-1.8ex]
...
\begin{table}[!htbp] \centering
\caption{Regression results: supervised machine learning methods }
\label{Tab:panelsml}
\resizebox{\columnwidth}{!}{
\begin{tabular}{@{\extracolsep{5pt}}lccc}
\\[-1.8ex]\hline
\hline \\[-1.8ex]
& \multicolumn{3}{c}{\textit{Dependent variable: $r_{i,t}$}} \
\cr \cline{3-4}
\\[-1.8ex] & (1) & (2) & (3) \\
\hline \\[-1.8ex]
  RF12 sentiment & 0.074$^{***}$ & & \\
  & (0.028 , 0.121) & & \\
  RFFin sentiment & & 0.192$^{***}$ & \\
  & & (0.166 , 0.219) & \\
  NNFin sentiment & & & 0.021$^{**}$ \\
  & & & (0.005 , 0.037) \\
  const & -9.0e-04$^{}$ & -9.0e-04$^{}$ & -10.0e-04$^{}$ \\
  & (-0.003 , 0.001) & (-0.003 , 0.001) & (-0.003 , 0.001) \\
  log(SIZE) & 3.6e-05$^{}$ & 3.5e-05$^{}$ & 3.9e-05$^{}$ \\
  & (-5.3e-05 , 0.000) & (-5.3e-05 , 0.000) & (-4.9e-05 , 0.000) \\
  btm & 2.3e-05$^{}$ & 2.4e-05$^{}$ & 2.3e-05$^{}$ \\
  & (-4.3e-05 , 8.9e-05) & (-4.2e-05 , 9.0e-05) & (-4.3e-05 , 8.9e-05) \\
  turn & -3.5e-05$^{}$ & -3.1e-05$^{}$ & -3.4e-05$^{}$ \\
  & (-0.000 , 3.0e-05) & (-9.6e-05 , 3.5e-05) & (-9.9e-05 , 3.1e-05) \\
  prealpha & 0.115$^{***}$ & 0.101$^{**}$ & 0.117$^{***}$ \\
  & (0.031 , 0.199) & (0.017 , 0.185) & (0.033 , 0.201) \\
  NASDAQ & 3.0e-04$^{***}$ & 3.0e-04$^{***}$ & 3.0e-04$^{***}$ \\
  & (0.000 , 0.001) & (10e-05 , 0.001) & (0.000 , 0.001) \\
\hline \\[-1.8ex]
 Observations & 111,300 & 111,300 & 111,300 \\
 $R^2$ & 0.029\% & 0.200\% & 0.026\% \\
 Adjusted $R^2$ & 0.024\% & 0.195\% & 0.021\% \\
 F Statistic & 5.455$^{***}$ (df = 6.0; 111293.0) & 37.150$^{***}$ (df = 6.0; 111293.0) & 4.892$^{***}$ (df = 6.0; 111293.0) \\
\hline
\hline \\[-1.8ex]
