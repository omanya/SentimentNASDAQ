...
- indicator of news arrival at $t$ $news_{i,t}$ and earnings announcements $ean_{i,t}$ as well 
- indicators of days before and after earnings announcements, $pre\_ean_{i,t}$ and  $post\_ean_{i,t}$. 

The last two together indicate a three-day period around the earnings announcements as in @BERKMAN2009. Share turnover is a known indicator of attention, as in @Loh2010.
...
Earnings surprises were used by @Maheu2004 to proxy investor sentiment. For dictionary based methods see e.g. @JEGADEESH2013. @FANG2021 uses machine learning methods to infer textual sentiment.
...
- the interaction term between small btm ratio and post earning day and 
...
In the following per-expectile distributional summary, the scale of the box plots is kept the same in the entire column for comparison purposes.
...
lbls<-c(paste("$r_{i,t}\\leq e_{",taus[1],"}$"),
        paste("$e_{",taus[1:(length(taus)-1)],"}\\leq r_{i,t}\\leq e_{",taus[-1],"}$"),paste("$r_{i,t}\\geq e_{",taus[length(taus)],"}$"))
...
coefss<-c(mod0$coefs["LM.TONE",1],mod0$coefs["HIV4.TONE",1], mod0$coefs["ML.TONE",1])
scoefs<-c(mod0$ses["LM.TONE",1], mod0$ses["HIV4.TONE",1],mod0$ses["ML.TONE",1])
lcoefs<-coefss-qnorm(1-al/2)*scoefs
hcoefs<-coefss+qnorm(1-al/2)*scoefs



mnews<-mod0$coefs["news",1]+sum(coefss*mposs)
lnews<-mod0$coefs["news",1]+ qnorm(1-al/2)*mod0$ses["news",1]+sum(lcoefs*mposs)
hnews<-mod0$coefs["news",1]- qnorm(1-al/2)*mod0$ses["news",1]+sum(hcoefs*mposs)

#whichc<-c(1,5)#e0.05,e0.25
#effects<-mod0$coefs["news",whichc] + mposlm[c(1,2)]*mod0$coefs["LM.TONE",whichc] + mposhv[c(1,2)]*mod0$coefs["HIV4.TONE",whichc] + mposml[c(1,2)]*mod0$coefs["ML.TONE",whichc]
