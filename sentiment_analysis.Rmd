...
We analyse how the distribution of excess returns is influenced by news arrivals and investor sentiment by applying the expectile panel data model with random effects to a cross section of $100$ excess equity returns in NASDAQ. We use firm specific news arrivals and assess the investor sentiment for the model by employing the dictionary method for extracting the individual news sentiment from NASDAQ news articles relevant to a particular equity.

We control for other investor opinion channels influencing returns by including proxies for investor attention, investor perception and investor sentiment pointed out in the previous related studies. 

We find, that various effects depend on the expectile level $\tau$, which itself can be understood as the 'state of the world' for individual equity returns. While not always significant for the mean regression, the proxies of investment opinion used impact the distribution of asset returns (in case of normality - it impacts the volatility) which we show empirically. 

When analysing the effect of news and earnings announcements, we find a positive impact on the day before earnings announcements arrival and a negative impact of news arrival on low return expectiles, which we explain with the strategy "buy on rumor, sell on news" for the low return states.
...
In the current paper we follow the classification of @Su2021 and incorporate all three associated channels of information flow influence on asset returns. That is, we include proxies for investor perception, investor attention, and investor sentiment in our analysis and assess the effects based on this structure, which makes our approach stand out from the previous research. 

Moreover, we conduct the effect analysis of information flow channels on the whole distribution of asset returns representing the later without imposing any distributional assumptions by a set of tail indices. 

A natural choice of such tail indices are quantiles (@Koenker1978) of the return, $q_{\tau}(r_{i,t})$, for a a range of $\tau$-values in $(0,1)$ capable of representing the whole return distribution without explicit distributional assumtions. By means of quantile regression distributional effects of investor opinions can be estimated. @NI2015, @Ma2018 and @ALNASSERI2021 use quantile regression to address the second raised issue: which distributional parameters of the return are effected. The authors find different effects for different quantile levels and point out that, overall, extreme markets are driven by investor sentiment.
...
We use data for NASDAQ stocks in the period April 2015 - August 2019 originated from Bloomberg. Our dependend variable is the excess return of NASDAQ stocks in the mentioned period, computed as...  {\color{red}(How we computed excess returns?)}. The associated data on size, btm ratios, share turnover, and the associated new articles build up the basis for explanatory variables.

We combine this data with the records on earnings announcements from https://www.kaggle.com/datasets/tsaustin/us-historical-stock-prices-with-earnings-data (accessed on 21.September 2022) and sector classification from NASDAQ website (https://www.nasdaq.com/market-activity/stocks/screener, accessed on 10. September 2022).
...
From the available data and based on the previous findings in the literature, we construct proxies for the three channels of information flow: investor attention, investor sentiment and investor perception described in the following.
...
# Expectile panel data regression 

In this section we review the general set up of our panel data model and its generalization to expectile regression (ER) and provide the assumptions underlying our analysis.

## General model definition

Consider a random effects model for the excess return ($r_{i,t}$) of firm $i$ at time $t$ with $i=1,\ldots,n$ and $t=1,\ldots, T$:
\[r_{i,t} = x_{i,t}^\top\beta + z_{i}^\top\gamma + \alpha_i + v_{i,t}\]

with response $r_{i,t}$ being the excess return of stock $i$ at time $t$ computed as specified in Section "Data". $x_{i,t}$ a vector of $p$ time varying predictors, and $z_i$ a vector of time-invariant covariates (including the constant) for the stock $i$ known at time $t$. $\alpha_i$ are individual specific effects uncorrelated with the predictors, $v_{i,t}$ is an error term uncorrelated with $x_{i,t}$ as well.

The model can be rewritten as:

\[r_{i,t} = \beta_0 + x_{i,t}^\top\beta + z_{i}^\top\gamma+u_{i,t},\]

where $u_{i,t} = \alpha_i + v_{it}$.  

The conditional expectile regression @Newey1987 aims at finding the conditional expectile (given the covariates $x_{i,t}$ and $z_i$):
...
We compute robust standard errors for the ER-coefficients with R-Package *sandwich* assuming cross-sectional error dependence within sectors and serial dependence within the individual stocks.
...
Many of the effects are insignificant for the mean return but become significant for $\tau$-values apart from $0.5$. That is, higher moments of the conditional distribution are affected by the considered investment opinion channel proxies. Also, the explained variance proportion becomes higher in the tails of the conditional return distribution. Moreover, some of the effects appear to be significant only for lower ($(btm_{i,t-1}<1)\cdot post\_ean_{i,t}$ and $hv_tone_{i,t}$) and some - only for higher expectiles  ($post\_ean_{i,t}$).


In the next figure, we plot the attention related effects with $95\%$-CI in Figure \@ref(fig:figure1):
...
```
The effects of $btm_{i,t-1}$ and of $\log(size_{i,t})$ appear to be positive for low expectiles and become increasingly negative for high $\tau$, indicating robustness of value stocks and large firms to extreme states of the market. Shares turnover around earnings announcements exhibits a negative effect on low and a positive effect on high expectiles. Retunrs of firms with small btm ratios (<1) and low return states react negatively in the post earning period.
...
```
The effect of earning surprise $surp_{i,t}$ is only for quite low expectiles significant. Whereas the effects of $lm\_tone_{i,t}$ and $ml\_tone_{i,t}$ do not differ significantly from a constant effect across expectiles, $hv\_tone_{i,t}$ seems to have an increasing impact on low return expectiles. Overall, the sentiment effects are positive, that is, the sign of the sentiment tone determines the expected direction of return reaction.
...
# Impact of investor opinion on expected shortfall of the returns

A frequently used risk measure is expected shortfall $ES_{\alpha}$ connected to a certain quantile $q_\alpha$ of the target distribution. For the lower tail of the later this risk measure is defined as $ES_\alpha = \mathbb E(Y|Y<q_\alpha)$. 
...
@yao1996 show the existence of a bijective function $\tau:(0,1)\rightarrow (0,1)$ such that $q_\alpha = e_{\tau(\alpha)}$.
...
Following the approach of @Taylor2008, the plug-in estimator for the expected conditional shortfall based on the estimated conditional expectile and the relation $\hat\tau(\alpha)$ in our case is:

\[\begin{align}
ES_{\alpha}(r_{i,t}|x_{i,t},z_i) &= \left(1+\frac{\hat\tau(\alpha)}{\big(1-2\hat\tau(\alpha)\big)\alpha}\right)e_{\hat\tau(\alpha)}(r_{i,t}|x_{i,t},z_i) - \frac{\hat\tau(\alpha)}{\big(1-2\hat\tau(\alpha)\big)\alpha}e_{0.5}(r_{i,t}|x_{i,t},z_i)(\#eq:es)\\
&=\tilde \beta_{0,\alpha} + \tilde\beta_{\alpha}^\top \cdot x_{i,t} + \tilde\gamma_\alpha^\top\cdot z_i,\nonumber
\end{align}\]
...
where for $\theta\in\{\beta_0,\beta, \gamma\}$:
...
To obtain the actual estimates, we first estimate the conditional expectiles of return as in \@ref(eq:mod0) for a fine grid of $\hat e_\tau$ with $\tau = 0.001, 0.0025, 0.005,0.01,0.02,\ldots, 0.1$. Based thereof, we construct an empirical cdf by counting the observations below the estimated expectile profile for each of the $\tau$-levels, such that for each $\tau$ we have the corresponding quantile index $\alpha$. We then interpolate linearly between the resulting points to obtain $\hat\tau(\alpha)$ particulary for $\alpha\in\{0.01,0.05,0.1\}$. The estimated correspondence is plotted in Figure \@ref(fig:figure4).
...
```{r, figure5, fig.cap="The estimated relation between the expectile level $\tau$ and the quantile level $\alpha$ for the NASDAQ return distribution with $95\%$ bootstrap confidence intervals", echo=FALSE}
library(scales)
plot(tau_alpha[,2], tau_alpha[,1],type="l",col=NA, ylab="", xlab="", axes=F, ylim=c(0,0.1), xlim=c(0,0.15))

axis(1, at=c(0,0.01,0.05,0.1,0.15), labels=c(0,0.01,0.05,0.1,0.15))
axis(2, las=1, at=c(0,0.05,0.1), labels=c(0,0.05,0.1))
par(xpd=T)

text(0,0.1,bquote(hat(tau)(alpha)))
text(0.15,0,bquote(alpha))

#text(0.01, tau_new$y[1]+0.01, bquote(hat(tau)(0.01)==0.0055),cex=0.7))
# for(i in 1:ncol(tau_alpha_b)){
#   lines(rownames(tau_alpha_b), tau_alpha_b[,i], col=alpha("gray44",0.3))
# }
lines(apply(tau_alpha_b,1,mean), rownames(tau_alpha_b), col=1,lwd=3,lty=3)
lines(apply(tau_alpha_b,1,quantile,0.025), rownames(tau_alpha_b), col=4,lty=2,lwd=3)
lines(apply(tau_alpha_b,1,quantile,0.975), rownames(tau_alpha_b), col=4,lty=2,lwd=3)
lines(tau_alpha[,2], tau_alpha[,1], lwd=3,col=alpha(2,0.8))
```
```{r, echo=FALSE}
#compute 95% ci
...
```
We can confirm significant effects of $pre\_ean$, $ean$, $news$, turnover around earnings announcement days $turn\cdot days\_ean$, post earnings announcements effect of small btm ratio $(btm\leq 1)\cdot post\_ean$. Neither $lm\_tone$ nor $ml\_tone$ show significant impact. However, for $hv\_tone$ we find a significant effect for $\alpha=0.05$ and $0.1$.
